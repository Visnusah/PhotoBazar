...
const Dashboard = () => {
  ...
  const fetchUserPhotos = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/photos/my-photos?t=${timestamp}`, {
        headers: {
  ...
  const handleDeletePhoto = async (photoId) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setPhotos(prevPhotos => prevPhotos.filter(photo => photo.id !== photoId)); // NEW - instant
        setStats(prevStats => ({ ...prevStats, totalPhotos: prevStats.totalPhotos - 1 })); // NEW - updates count
      } else {
            ...
            {photos.map((photo, index) => (
              ...
              >
                <div className="aspect-w-16 aspect-h-9">
                  <img
                    src={getImageUrlWithToken(photo.imageUrl)}
                    alt={photo.title}
